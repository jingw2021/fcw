...
def ttc_cal(dataDQ, fps):
    ...
    matches = match_descriptors(
        prev_frame['descriptor'], cur_frame['descriptor'], "FLANN")
    # keep only the common tracks
    common_trks = np.intersect1d(
        prev_frame['tracks'][:, -1], cur_frame['tracks'][:, -1])
    prev_idx = [
        True if ele in common_trks else False for ele in prev_frame['tracks'][:, -1]]
    cur_idx = [
        True if ele in common_trks else False for ele in cur_frame['tracks'][:, -1]]
    ...
    for trk_index in common_trks:
        ...
        for outer_idx in range(len(trk_matches)-1):
            kp_outer_prev = prev_frame['keypoints'][matches[int(
                trk_matches[outer_idx][1])][0].queryIdx].pt
            kp_outer_cur = cur_frame['keypoints'][matches[int(
                trk_matches[outer_idx][1])][0].trainIdx].pt
            for inner_idx in range(outer_idx+1, len(trk_matches)):
                kp_inner_prev = prev_frame['keypoints'][matches[int(
                    trk_matches[inner_idx][1])][0].queryIdx].pt
                kp_inner_cur = cur_frame['keypoints'][matches[int(
                    trk_matches[inner_idx][1])][0].trainIdx].pt
        ...
        if len(dist_ratio) > 0:
            if np.median(dist_ratio) == 1:
                ...
                ttc = d_t/(np.median(dist_ratio)-1)
        else:
            ttc = -1
