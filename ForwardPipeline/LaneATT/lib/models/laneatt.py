...
from .matching import INFINITY, match_proposals_with_targets
...
class LaneATT(nn.Module):
    ...
    def override_nms(self, proposals, scores, overlap, top_k):
        def distance(p1, p2):
            start1 = int(p1[2] * self.n_strips + 0.5)
            start2 = int(p2[2] * self.n_strips + 0.5)
            end1 = int(start1 + p1[4] -1 + 0.5 - int((p1[4] - 1) < 0))
            end2 = int(start2 + p2[4] -1 + 0.5 - int((p2[4] - 1) < 0))
            start = max(start1, start2)
            end = min(min(end1, end2), self.n_offsets - 1)

            if end < start:
                return 10**5 # INFINITY
            else:
                distance = (p1[start + 5:end + 6] - p2[start + 5 :end + 6]).abs()
                return distance.mean()

        keep = []

        order = np.argsort(scores).numpy()[::-1].copy()

        remaining_sorted_proposals = [(order[i], proposal) for i, proposal in enumerate(proposals[order])]

        
        while len(remaining_sorted_proposals) > 0:
            keep_next_stage = []
            i, proposal = remaining_sorted_proposals[0]
            keep.append(i)
            
            for candidate in remaining_sorted_proposals[1:]:
                d = distance(proposal, candidate[1])
                if d > overlap:
                    keep_next_stage.append(candidate)
            remaining_sorted_proposals = keep_next_stage
        ...
        for proposals, attention_matrix in zip(batch_proposals, batch_attention_matrix):
            ...
            with torch.no_grad():
                ...
                if proposals.shape[0] == 0:
                    ...
                    continue
                keep, num_to_keep, _ = self.override_nms(proposals, scores, overlap=nms_thres, top_k=nms_topk)
